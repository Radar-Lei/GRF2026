...
var oklch = function (context, args) {
    ...
    var l = length_percentage_1.isLengthPercentage(tokens[0]) ? tokens[0].number / 100 : tokens[0].number;
    ...
    var h_deg;
    if (hueToken.type === 17 /* NUMBER_TOKEN */) {
        h_deg = angle_1.deg(hueToken.number);
    }
    else {
        h_deg = angle_1.angle.parse(context, hueToken);
    }
    // Optional alpha value.
    var a = tokens.length >= 4 ? (length_percentage_1.isLengthPercentage(tokens[3]) ? length_percentage_1.getAbsoluteValue(tokens[3], 1) : tokens[3].number) : 1;
    var _a = oklchToSRGB(l, c, h_deg), r = _a[0], g = _a[1], b = _a[2];
    return exports.pack(r, g, b, a);
};
